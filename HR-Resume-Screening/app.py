import streamlit as st
import pandas as pd
import plotly.graph_objects as go

from config import APP_TITLE, RECOMMENDATION_THRESHOLD
from src.resume_parser import parse_resume
from src.feature_engineering import build_feature_vector, feature_dict_to_array
from src.prediction_service import predict_fit_probability, ensure_model_ready
from src.explainability import explain_prediction, compute_confidence, readable_feature_name
from src.llm_service import generate_explanation_narrative, generate_report_narrative, analyze_candidate_resume
from src.database import (
    init_db,
    insert_candidate,
    update_hitl_decision,
    get_all_candidates,
    clear_all_candidates,
)
from src.report_generator import generate_pdf_report
from src.utils import generate_id, parse_json_field

# ---------------------------------------------------------------------------
# PAGE CONFIG & INITIAL SETUP
# ---------------------------------------------------------------------------
st.set_page_config(page_title=APP_TITLE, page_icon="🧑‍💼", layout="wide")

init_db()

if "model_ready" not in st.session_state:
    with st.spinner("Preparing AI model (first-time setup, training ANN on synthetic data)..."):
        ensure_model_ready()
    st.session_state["model_ready"] = True

if "jd_text" not in st.session_state:
    st.session_state["jd_text"] = ""

def _load_shap_data(cand: dict) -> dict:
    explanation_data = parse_json_field(cand.get("shap_contributions_json"), default=None)
    if isinstance(explanation_data, dict) and "contributions" in explanation_data:
        return explanation_data

    features = parse_json_field(cand.get("features_json"), default={})
    if not isinstance(features, dict) or not features:
        return {"contributions": []}
    feature_array = feature_dict_to_array(features)
    return explain_prediction(feature_array)


# ---------------------------------------------------------------------------
# SIDEBAR NAVIGATION
# ---------------------------------------------------------------------------
st.sidebar.title("HR Resume Copilot")
page = st.sidebar.radio(
    "Navigate",
    [
        "1. Upload & Screen",
        "2. Human-in-the-Loop Review",
        "3. Analytics Dashboard",
        "4. Generate Report",
    ],
)

st.sidebar.markdown("---")
st.sidebar.caption("HR Resume Screening")

st.sidebar.subheader("Danger Zone")
confirm_clear = st.sidebar.checkbox("⚠️ Enable Data Clearing", value=False)
if confirm_clear:
    if st.sidebar.button("🗑️ Clear All Data Now", type="primary", use_container_width=True):
        clear_all_candidates()
        st.sidebar.success("All candidate data cleared.")
        st.rerun()


# ---------------------------------------------------------------------------
# PAGE 1: UPLOAD & SCREEN
# ---------------------------------------------------------------------------
if page == "1. Upload & Screen":
    st.title(APP_TITLE)
    st.write(
        "Paste or upload the job description and candidate resumes. "
        "The system will analyze each resume, compare it with the job requirements, "
        "and rank the candidates based on their suitability."
    )

    col1, col2 = st.columns([1, 1])

    with col1:
        st.subheader("Step 1: Job Description")
        jd_paste = st.text_area(
            "Paste Job Description",
            value=st.session_state["jd_text"],
            height=180,
            placeholder="Paste the full job description here...",
            key="jd_paste_input",
        )
        if jd_paste.strip():
            st.session_state["jd_text"] = jd_paste

        jd_file = st.file_uploader("Or upload Job Description (PDF)", type=["pdf"])
        if jd_file is not None:
            st.session_state["jd_text"] = parse_resume(jd_file.name, jd_file.read())

    with col2:
        st.subheader("Step 2: Upload Resumes")
        resume_files = st.file_uploader(
            "Upload candidate resumes (PDF only) — multiple files allowed",
            type=["pdf"],
            accept_multiple_files=True,
        )

    st.markdown("---")

    if st.button("🚀 Run AI Screening", type="primary", use_container_width=True):
        if not st.session_state["jd_text"].strip():
            st.error("Please provide a Job Description first.")
        elif not resume_files:
            st.error("Please upload at least one resume.")
        else:
            jd_text = st.session_state["jd_text"]
            progress = st.progress(0, text="Starting screening...")
            processed_count = 0

            for idx, resume_file in enumerate(resume_files):
                progress.progress(
                    (idx) / len(resume_files),
                    text=f"Processing {resume_file.name} ({idx + 1}/{len(resume_files)})...",
                )

                resume_bytes = resume_file.read()
                resume_text = parse_resume(resume_file.name, resume_bytes)

                if not resume_text.strip():
                    st.warning(f"Could not extract text from {resume_file.name}. Skipping.")
                    continue

                feature_result = build_feature_vector(resume_text, jd_text)
                features = feature_result["features"]
                meta = feature_result["meta"]
                feature_array = feature_dict_to_array(features)

                probability = predict_fit_probability(feature_array)
                confidence = compute_confidence(probability)
                recommendation = "Select" if probability >= RECOMMENDATION_THRESHOLD else "Reject"

                explanation_data = explain_prediction(feature_array)

                candidate_name = resume_file.name.rsplit(".", 1)[0].replace("_", " ").title()
                llm_analysis = analyze_candidate_resume(resume_text, jd_text)
                llm_summary = llm_analysis.get("summary", "")

                llm_explanation = generate_explanation_narrative(
                    candidate_name=candidate_name,
                    fit_probability=probability,
                    confidence=confidence,
                    top_contributions=explanation_data["contributions"],
                    matched_skills=meta["matched_skills"],
                    missing_skills=meta["missing_skills"],
                )

                candidate_id = generate_id("CAND")
                record = {
                    "candidate_id": candidate_id,
                    "candidate_name": candidate_name,
                    "file_name": resume_file.name,
                    "resume_text": resume_text,
                    "jd_snapshot": jd_text[:2000],
                    "features": features,
                    "matched_skills": meta["matched_skills"],
                    "missing_skills": meta["missing_skills"],
                    "ai_probability": probability,
                    "ai_confidence": confidence,
                    "ai_recommendation": recommendation,
                    "llm_summary": llm_summary,
                    "llm_explanation": llm_explanation,
                    "llm_strengths": llm_analysis.get("strengths", ""),
                    "llm_weaknesses": llm_analysis.get("weaknesses", ""),
                    "llm_missing_skills_explanation": llm_analysis.get("missing_skills_explanation", ""),
                    "llm_interview_questions": llm_analysis.get("interview_questions", ""),
                    "shap_contributions": explanation_data,
                    "hitl_decision": "Pending",
                }
                insert_candidate(record)
                processed_count += 1

            progress.progress(1.0, text="Screening complete!")
            st.success(
                f"Successfully screened {processed_count} candidate(s). "
                "Review ranked results below or go to 'Human-in-the-Loop Review'."
            )

            ranked_candidates = get_all_candidates()
            if ranked_candidates:
                st.subheader("Ranked Candidates")
                ranked_rows = []
                for rank, cand in enumerate(ranked_candidates, start=1):
                    ranked_rows.append(
                        {
                            "Rank": rank,
                            "Candidate": cand["candidate_name"],
                            "Fit Score (%)": round(cand["ai_probability"] * 100, 1),
                            "Confidence (%)": cand["ai_confidence"],
                            "AI Recommendation": cand["ai_recommendation"],
                            "HR Decision": cand["hitl_decision"],
                        }
                    )
                st.dataframe(pd.DataFrame(ranked_rows), use_container_width=True, hide_index=True)


# ---------------------------------------------------------------------------
# PAGE 2: HUMAN IN THE LOOP REVIEW
# ---------------------------------------------------------------------------
elif page == "2. Human-in-the-Loop Review":
    st.title("HR Review")
    st.write("Review each AI recommendation and Approve, Reject, or Hold the decision.")

    candidates = get_all_candidates()

    if not candidates:
        st.info("No candidates screened yet. Go to 'Upload & Screen' first.")
    else:
        for rank, cand in enumerate(candidates, start=1):
            active_fit_score = (
                cand.get("hitl_modified_score")
                if cand.get("hitl_modified_score") is not None
                else cand["ai_probability"]
            )
            rec_icon = "✅" if cand["ai_recommendation"] == "Select" else "⚠️"
            st_header = (
                f"#{rank} {rec_icon} {cand['candidate_name']}  —  "
                f"Fit: {active_fit_score * 100:.1f}%  | "
                f"Confidence: {cand['ai_confidence']}%  | "
                f"AI: {cand['ai_recommendation']}  | Status: {cand['hitl_decision']}"
            )

            with st.expander(st_header):
                col1, col2 = st.columns([1.3, 1])

                with col1:
                    st.markdown("**🧠 LLM Resume Summary**")
                    st.write(cand.get("llm_summary", ""))

                    sc1, sc2 = st.columns(2)
                    with sc1:
                        st.markdown("**⭐ Candidate Strengths**")
                        st.write(cand.get("llm_strengths", "No strengths listed."))
                    with sc2:
                        st.markdown("**⚠️ Areas for Improvement / Weaknesses**")
                        st.write(cand.get("llm_weaknesses", "No weaknesses listed."))

                    st.markdown("**🔍 Missing Skills Explanation**")
                    st.write(cand.get("llm_missing_skills_explanation", "No missing skills explanation."))

                    st.markdown("**💬 Suggested Interview Questions**")
                    st.write(cand.get("llm_interview_questions", "No suggested questions."))

                    st.markdown("**💡 AI Explanation (Narrative)**")
                    st.write(cand["llm_explanation"])

                    matched = parse_json_field(cand.get("matched_skills_json"), default=[])
                    missing = parse_json_field(cand.get("missing_skills_json"), default=[])
                    st.markdown(f"**Matched Skills:** {', '.join(matched) if matched else 'None'}")
                    st.markdown(f"**Missing Skills:** {', '.join(missing) if missing else 'None'}")

                with col2:
                    st.markdown("**📊 Feature Contribution (SHAP)**")
                    explanation_data = _load_shap_data(cand)

                    fig = go.Figure(
                        go.Bar(
                            x=[c["shap_contribution"] for c in explanation_data["contributions"]],
                            y=[readable_feature_name(c["feature"]) for c in explanation_data["contributions"]],
                            orientation="h",
                            marker_color=[
                                "#16A34A" if c["shap_contribution"] >= 0 else "#DC2626"
                                for c in explanation_data["contributions"]
                            ],
                        )
                    )
                    fig.update_layout(
                        height=320,
                        margin=dict(l=10, r=10, t=10, b=10),
                        xaxis_title="Impact on Fit Score",
                    )
                    st.plotly_chart(fig, use_container_width=True, key=f"chart_{cand['candidate_id']}")

                st.markdown("---")
                st.markdown("**✅ Human-in-the-Loop Decision**")

                notes = st.text_input(
                    "HR Notes / Reason",
                    value=cand.get("hitl_notes") or "",
                    key=f"notes_{cand['candidate_id']}",
                )
                modified_score = st.slider(
                    "Modify Fit Score Manually",
                    0.0,
                    100.0,
                    float(cand.get("hitl_modified_score") or cand["ai_probability"]) * 100,
                    key=f"slider_{cand['candidate_id']}",
                )

                current_notes = st.session_state.get(f"notes_{cand['candidate_id']}", notes)
                current_score = st.session_state.get(f"slider_{cand['candidate_id']}", modified_score) / 100.0

                hitl_col1, hitl_col2, hitl_col3, hitl_col4 = st.columns(4)
                with hitl_col1:
                    if st.button("✅ Approve", key=f"approve_{cand['candidate_id']}", use_container_width=True):
                        update_hitl_decision(
                            cand["candidate_id"], "Approved", notes=current_notes, modified_score=current_score
                        )
                        st.success("Candidate Approved!")
                        st.rerun()
                with hitl_col2:
                    if st.button("❌ Reject", key=f"reject_{cand['candidate_id']}", use_container_width=True):
                        update_hitl_decision(
                            cand["candidate_id"], "Rejected", notes=current_notes, modified_score=current_score
                        )
                        st.warning("Candidate Rejected.")
                        st.rerun()
                with hitl_col3:
                    if st.button("⏸️ Hold", key=f"hold_{cand['candidate_id']}", use_container_width=True):
                        update_hitl_decision(
                            cand["candidate_id"], "Hold", notes=current_notes, modified_score=current_score
                        )
                        st.info("Candidate placed on Hold.")
                        st.rerun()
                with hitl_col4:
                    if st.button("💾 Save Modified Score", key=f"modify_{cand['candidate_id']}", use_container_width=True):
                        update_hitl_decision(
                            cand["candidate_id"], "Modified", notes=current_notes, modified_score=current_score
                        )
                        st.success("Modified score saved.")
                        st.rerun()


# ---------------------------------------------------------------------------
# PAGE 3: ANALYTICS DASHBOARD
# ---------------------------------------------------------------------------
elif page == "3. Analytics Dashboard":
    st.title("Analytics Dashboard")

    candidates = get_all_candidates()

    if not candidates:
        st.info("No candidates screened yet.")
    else:
        df = pd.DataFrame(candidates)
        df["rank"] = range(1, len(df) + 1)

        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("Total Candidates", len(df))
        c2.metric("AI Select", int((df["ai_recommendation"] == "Select").sum()))
        c3.metric("HR Approved", int((df["hitl_decision"] == "Approved").sum()))
        c4.metric("On Hold", int((df["hitl_decision"] == "Hold").sum()))
        c5.metric("Avg. Confidence", f"{df['ai_confidence'].mean():.1f}%")

        st.markdown("---")

        col1, col2 = st.columns(2)
        with col1:
            st.subheader("AI Recommendation Breakdown")
            rec_counts = df["ai_recommendation"].value_counts()
            fig1 = go.Figure(go.Pie(labels=rec_counts.index, values=rec_counts.values, hole=0.4))
            st.plotly_chart(fig1, use_container_width=True)

        with col2:
            st.subheader("HR Decision Breakdown")
            dec_counts = df["hitl_decision"].value_counts()
            fig2 = go.Figure(go.Pie(labels=dec_counts.index, values=dec_counts.values, hole=0.4))
            st.plotly_chart(fig2, use_container_width=True)

        st.subheader("Candidate Fit Scores (Ranked)")
        fig3 = go.Figure(
            go.Bar(x=df["candidate_name"], y=df["ai_probability"] * 100, marker_color="#2563EB")
        )
        fig3.update_layout(yaxis_title="Fit Probability (%)", xaxis_title="Candidate")
        st.plotly_chart(fig3, use_container_width=True)

        st.subheader("Raw Data Table")
        display_cols = [
            "rank", "candidate_name", "file_name", "ai_recommendation",
            "ai_probability", "ai_confidence", "hitl_decision", "hitl_notes",
        ]
        st.dataframe(df[display_cols], use_container_width=True, hide_index=True)


# ---------------------------------------------------------------------------
# PAGE 4: GENERATE REPORT
# ---------------------------------------------------------------------------
elif page == "4. Generate Report":
    st.title("Generate Downloadable Report")

    candidates = get_all_candidates()

    if not candidates:
        st.info("No candidates screened yet. Nothing to report.")
    else:
        total = len(candidates)
        selected = sum(1 for c in candidates if c["ai_recommendation"] == "Select")
        approved = sum(1 for c in candidates if c["hitl_decision"] == "Approved")
        rejected = sum(1 for c in candidates if c["hitl_decision"] == "Rejected")
        on_hold = sum(1 for c in candidates if c["hitl_decision"] == "Hold")
        modified = sum(1 for c in candidates if c["hitl_decision"] == "Modified")
        avg_confidence = round(sum(c["ai_confidence"] for c in candidates) / total, 1)

        summary_stats = {
            "total": total,
            "selected": selected,
            "rejected_ai": total - selected,
            "approved": approved,
            "rejected": rejected,
            "hold": on_hold,
            "modified": modified,
            "avg_confidence": avg_confidence,
        }

        st.write("Summary that will be included in the report:")
        st.json(summary_stats)

        if st.button("📝 Generate Executive Summary (LLM)", type="primary"):
            with st.spinner("Generating executive summary using LLM..."):
                narrative = generate_report_narrative(summary_stats)
            st.session_state["executive_summary"] = narrative

        executive_summary = st.session_state.get(
            "executive_summary", "Click the button above to generate an AI-written executive summary."
        )
        st.text_area("Executive Summary (editable)", value=executive_summary, height=150, key="exec_summary_box")

        if st.button("📄 Build PDF Report", use_container_width=True):
            with st.spinner("Building PDF report..."):
                st.session_state["pdf_report_bytes"] = generate_pdf_report(
                    candidates,
                    st.session_state.get("exec_summary_box", executive_summary),
                    summary_stats,
                )
            st.success("PDF report ready for download.")

        if st.session_state.get("pdf_report_bytes"):
            st.download_button(
                label="⬇️ Download PDF Report",
                data=st.session_state["pdf_report_bytes"],
                file_name="HR_Resume_Screening_Report.pdf",
                mime="application/pdf",
                use_container_width=True,
            )
