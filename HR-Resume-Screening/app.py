...
from src.llm_service import summarize_resume, generate_explanation_narrative, generate_report_narrative, analyze_candidate_resume
...
if page == "1. Upload & Screen":
    st.title(APP_TITLE)
    st.write(
        "Upload the job description and candidate resumes."
        "The system will analyze each resume, compare it with the job requirements, and rank the candidates based on their suitability."
    )
    ...
    with col1:
        ...
        if st.session_state.get("jd_text"):
            st.text_area("Extracted Job Description Text", value=st.session_state["jd_text"], height=220, disabled=True)

    with col2:
        st.subheader("Step 2: Upload Resumes")
        resume_files = st.file_uploader(
            "Upload candidate resumes (PDF/DOCX) — multiple files allowed",
            type=["pdf", "docx"],
    ...
    if st.button("🚀 Run AI Screening", type="primary", use_container_width=True):
        if not st.session_state["jd_text"].strip():
            ...
            for idx, resume_file in enumerate(resume_files):
                ...
                # ---- LLM Reasoning: summary, strengths, weaknesses, missing skills, questions ----
...
# ---------------------------------------------------------------------------
    ...
    if not candidates:
        ...
        for cand in candidates:
            active_fit_score = cand.get("hitl_modified_score") if cand.get("hitl_modified_score") is not None else cand["ai_probability"]
            st_header = (
                f"{'✅' if cand['ai_recommendation']=='Recommended' else '⚠️'} "
                f"{cand['candidate_name']}  —  Fit: {active_fit_score*100:.1f}%  "
            ...
            with st.expander(st_header):
                ...
                with col1:
                    ...
                    # Side-by-side strengths and weaknesses columns
                ...
                with col2:
                    ...
                    if "shap_contributions_json" in cand and cand["shap_contributions_json"]:
                        try:
                            explanation_data = json.loads(cand["shap_contributions_json"])
                        except Exception:
                            explanation_data = None
                ...
                current_notes = st.session_state.get(f"notes_{cand['candidate_id']}", cand.get("hitl_notes") or "")
                current_score = st.session_state.get(f"slider_{cand['candidate_id']}", float(cand.get("hitl_modified_score") or cand["ai_probability"]) * 100) / 100.0
                ...
                with hitl_col1:
                    if st.button("✅ Approve", key=f"approve_{cand['candidate_id']}", use_container_width=True):
                        update_hitl_decision(cand["candidate_id"], "Approved", notes=current_notes, modified_score=current_score)
                        st.success("Candidate Approved!")
                        st.rerun()
                with hitl_col2:
                    if st.button("❌ Reject", key=f"reject_{cand['candidate_id']}", use_container_width=True):
                        update_hitl_decision(cand["candidate_id"], "Rejected", notes=current_notes, modified_score=current_score)
                        st.warning("Candidate Rejected.")
                        st.rerun()
                with hitl_col3:
                    modified_score = st.slider(
                        "Modify Fit Score Manually",
                        0.0, 100.0, float(cand.get("hitl_modified_score") or cand["ai_probability"]) * 100,
