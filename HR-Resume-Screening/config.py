...
config.py
---------
Central configuration file for the HR Resume Screening Copilot.
...
# Load local .env (ignored on Streamlit Cloud because secrets are preferred)
load_dotenv()
...
def get_secret(key: str, default: str = "") -> str:
    """
    Reads values from Streamlit Secrets when deployed.
    Falls back to .env / environment variables during local development.
    """
    try:
...
# Candidates with probability >= threshold are recommended
RECOMMENDATION_THRESHOLD = 0.5

# Feature vector order expected by the ANN model
FEATURE_NAMES = [
    "skill_match_score",
