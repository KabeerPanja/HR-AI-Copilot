...
def generate_report_narrative(summary_stats: dict) -> str:
    ...
    user_prompt = (
        ...
        f"AI Select recommendations: {summary_stats.get('selected', summary_stats.get('recommended', 0))}\n"
        f"AI Reject recommendations: {summary_stats.get('rejected_ai', summary_stats.get('not_recommended', 0))}\n"
        f"HR Approved: {summary_stats.get('approved')}\n"
        f"HR Rejected: {summary_stats.get('rejected')}\n"
        f"HR Hold: {summary_stats.get('hold', 0)}\n"
        f"HR Modified scores: {summary_stats.get('modified')}\n"
