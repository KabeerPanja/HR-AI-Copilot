...
report_generator.py
--------------------
This module generates a downloadable PDF report containing:
    - Executive Summary
...
def generate_pdf_report(candidates: list, executive_summary: str, summary_stats: dict) -> bytes:
    """
    Generates the complete screening report as a PDF and returns it as bytes.
    """
    buffer = BytesIO()
    ...
    stats_data = [
        ["Metric", "Value"],
        ["Total Candidates", str(summary_stats.get("total", 0))],
    ...
    # ------------------------------------------------------------------
    elements.append(
        Paragraph("Candidate-wise Screening Results", styles["SectionHeading"])
    )

    table_data = [
        ["Candidate", "AI Decision", "Fit %", "Confidence", "HR Decision"]
    ]
    ...
    candidate_table = Table(
        table_data,
        colWidths=[5.5 * cm, 3 * cm, 2.3 * cm, 2.7 * cm, 3 * cm],
    )
    ...
    elements.append(
        Paragraph("Detailed AI Explanations", styles["SectionHeading"])
    )

    for cand in candidates:
        elements.append(
            Paragraph(
                f"<b>{cand.get('candidate_name', 'N/A')}</b> — {cand.get('file_name', '')}",
                styles["Heading4"],
            )
        )
        ...
        if cand.get("hitl_notes"):
            elements.append(
                Paragraph(
                    f"<i>HR Notes: {cand['hitl_notes']}</i>",
                    styles["BodySmall"],
                )
            )
