...
def init_db():
    """Database aur tables create karta hai agar exist nahi karte. Naye columns safely add karta hai."""
    with get_connection() as conn:
        ...
        # Safe migration logic for new columns
        new_columns = {
            "llm_strengths": "TEXT DEFAULT ''",
            "llm_weaknesses": "TEXT DEFAULT ''",
            "llm_missing_skills_explanation": "TEXT DEFAULT ''",
            "llm_interview_questions": "TEXT DEFAULT ''",
            "shap_contributions_json": "TEXT DEFAULT '[]'"
        }
...
def _serialize_json(value) -> str:
    """Serialize dict/list fields once for SQLite storage."""
    ...
    with get_connection() as conn:
        ...
        cursor.execute(
            ...
            INSERT OR REPLACE INTO candidates (
                ...
                hitl_modified_score, llm_strengths, llm_weaknesses,
                llm_missing_skills_explanation, llm_interview_questions,
                shap_contributions_json, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                record["candidate_id"],
                ...
                record["resume_text"],
                record["jd_snapshot"],
                _serialize_json(record["features"]),
                _serialize_json(record["matched_skills"]),
                _serialize_json(record["missing_skills"]),
                ...
                record.get("llm_strengths", ""),
                record.get("llm_weaknesses", ""),
                record.get("llm_missing_skills_explanation", ""),
                record.get("llm_interview_questions", ""),
                _serialize_json(record.get("shap_contributions", {})),
                current_timestamp(),
                current_timestamp(),
            ),
        )
...
def update_hitl_decision(candidate_id: str, decision: str, notes: str = "", modified_score: float = None):
    """HR reviewer ka HITL decision (Approve/Reject/Hold/Modified) update karta hai."""
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            """
...
def get_all_candidates() -> list:
    """Candidates ko unke match score (modified score, warna AI probability) ke descending order mein return karta hai."""
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            """
            SELECT * FROM candidates 
            ORDER BY COALESCE(hitl_modified_score, ai_probability) DESC
            """
        )
